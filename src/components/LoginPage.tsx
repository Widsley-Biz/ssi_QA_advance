        ...
        {isSupabaseMode ? (
          ...
          <>
            <p style={{ fontSize: 13, color: '#888', margin: '0 0 24px', lineHeight: 1.6 }}>
              @widsley.com のGoogleアカウントでログインしてください
            </p>
            <button
          ...
          <>
            <div style={styles.selectWrapper}>
              <label style={styles.label}>デモモード：ログインするユーザーを選択</label>
              <select
                value={selectedId}
              ...
              >
                ...
                {profiles.filter(p => p.role !== 'retired').map((p) => {
                  const team = teams.find((t) => t.id === p.team_id);
                  return (
                    <option key={p.id} value={p.id}>
                      {p.display_name} ({roleLabelMap[p.role] ?? p.role} / {team?.name ?? '未割当'})
                    </option>
                  );
                })}
              </select>
                  ...
                  return (
                    <>
                      ...
                      <div style={styles.previewDetail}>
                        <span style={styles.roleBadge}>{roleLabelMap[p.role] ?? p.role}</span>
                        <span>{team?.name ?? '未割当'}</span>
