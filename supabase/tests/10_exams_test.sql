...
INSERT INTO exam_questions (exam_id, no, category, question, choices, correct_key, explanation, points, difficulty) VALUES
  ('t-fixed', 1, 'locator', 'Q1 正しいlocatorはどれか', '{"a":"getByRole","b":"findElement","c":"querySelector","d":"$x"}', 'a', 'Playwrightは getByRole を推奨', 5, 1),
  ('t-fixed', 2, 'git',     'Q2 PRを出す直前のコマンドは', '{"a":"git init","b":"git push","c":"git clone","d":"git rm"}',    'b', 'ローカルコミット後にpush', 10, 2),
  ('t-fixed', 3, 'AAA',     'Q3 AAAのAssertはどれ',      '{"a":"page.goto","b":"page.click","c":"expect(...)","d":"beforeEach"}', 'c', 'Assert は expect', 15, 3),
  ('t-shuf',  1, 'locator', 'S1', '{"a":"1","b":"2"}', 'a', '', 10, 1),
  ('t-shuf',  2, 'locator', 'S2', '{"a":"1","b":"2"}', 'b', '', 10, 1),
  ('t-shuf',  3, 'locator', 'S3', '{"a":"1","b":"2"}', 'a', '', 10, 1),
  ('t-draft', 1, 'x',       'D1', '{"a":"1","b":"2"}', 'a', '', 10, 1);

\echo '=== 1. CHECK制約: correct_key が choices に無い設問は弾かれるか ==='
DO $$ BEGIN
  BEGIN
    INSERT INTO exam_questions (exam_id, no, question, choices, correct_key, points)
    VALUES ('t-fixed', 99, 'bad', '{"a":"1","b":"2"}', 'z', 1);
    RAISE EXCEPTION 'FAIL: correct_key が choices に無いのに挿入できた';
  EXCEPTION WHEN check_violation THEN
    RAISE NOTICE 'OK: CHECK制約が機能（correct_key=z を拒否）';
  END;
  BEGIN
    INSERT INTO exam_questions (exam_id, no, question, choices, correct_key, points)
...
DECLARE v jsonb; v_txt text; BEGIN
  ...
  v_txt := v::text;
  IF v_txt LIKE '%correct_key%' THEN RAISE EXCEPTION 'FAIL: correct_key が漏れている'; END IF;
...
DECLARE aid bigint; ans jsonb := '{}'; r jsonb; q jsonb; BEGIN
  ...
  FOR q IN SELECT * FROM jsonb_array_elements(public.start_exam('t-fixed')->'questions') LOOP
    ans := ans || jsonb_build_object(
      q->>'id',
      CASE q->>'no' WHEN '1' THEN 'a' WHEN '2' THEN 'd' WHEN '3' THEN 'c' END
    );
  END LOOP;
...
DECLARE aid bigint; r jsonb; BEGIN
  ...
  IF (r->'results'->0->>'selected_key') IS NOT NULL THEN RAISE EXCEPTION 'FAIL: 未回答がNULLでない'; END IF;
  RAISE NOTICE 'OK: 全問未回答→0/30・不合格・selected_keyはNULL';
END $$;
